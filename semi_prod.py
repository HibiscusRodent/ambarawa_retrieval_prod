...
output_folder_path: str = "data/output_book_data"
...
book_folder: BookFolderPathData = BookFolderPathData(path=sample_book_folder_path)
images_data: ProcessedBookData = ip.process_book_folder(book_folder)

# store the images data in various formats into variables
book_id: str = images_data.book_id
baml_images = images_data.baml_images
binary_images: list[bytes] = images_data.binary_images

# file write operations
## creating a folder for the specific book id
book_output_folder: str = os.path.join(output_folder_path, book_id)
...
book_condition_data: BookConditionData = b.GetBookConditionData(
    MultiImages=baml_images, bookId=book_id
)

# save the BookConditionData as json
book_condition_data_json_path: str = os.path.join(
    book_output_folder, f"{book_id}_book_condition_data.json"
)
...
bookRawAnalysis: RawAnalysis = b.GetBookrawVisual(
    MultiImages=baml_images, bookId=book_id
)

# save the RawAnalysis as json
raw_analysis_json_path: str = os.path.join(
    book_output_folder, f"{book_id}_raw_analysis.json"
)
...
book_content_hints: BookContentHints = b.GetBookContentHints(
...
book_content_hints_json_path: str = os.path.join(
    book_output_folder, f"{book_id}_book_content_hints.json"
)
...
# get bookMainData
book_main_data: BookMainData = b.GetBookMainData(
    MultiImages=baml_images,
    bookId=book_id,
    RawVisualNote=bookRawAnalysis.model_dump_json(),
)
...
book_main_data_json_path: str = os.path.join(
    book_output_folder, f"{book_id}_book_main_data.json"
)
...
book_pub_and_dist_details: BookPubAndDistDetails = b.GetBookPublisherData(
    MultiImages=baml_images,
    bookId=book_id,
    RawVisualNote=bookRawAnalysis.model_dump_json(),
)
...
book_pub_and_dist_details_json_path: str = os.path.join(
    book_output_folder, f"{book_id}_book_pub_and_dist_details.json"
)
with open(book_pub_and_dist_details_json_path, "w", encoding="utf-8") as json_file:
    json_file.write(
        book_pub_and_dist_details.model_dump_json(indent=4, ensure_ascii=False)
    )
...
book_main_dumped: dict[str, Any] = book_main_data.model_dump()
book_pub_dumped: dict[str, Any] = book_pub_and_dist_details.model_dump()
book_condition_dumped: dict[str, Any] = book_condition_data.model_dump()
book_content_dumped: dict[str, Any] = book_content_hints.model_dump()
...
data_to_ingest = [
    {
        ...
        "authors": book_main_dumped.get(
            "authors", []
        ),  # List of dicts with labeled fields
        "translators": book_main_dumped.get(
            "translator", []
        ),  # List of dicts with labeled fields
        ...
        "publisher_name": book_pub_dumped.get(
            "publisher_name", {}
        ),  # Dict with labeled fields
        "publisher_location": book_pub_dumped.get(
            "publisher_location", {}
        ),  # Dict with labeled fields
        "distributor_name": book_pub_dumped.get(
            "distributor_name", {}
        ),  # Dict with labeled fields
        "distributor_location": book_pub_dumped.get(
        ...
        "isFiction": book_content_dumped.get(
            "isFiction", False
        ),  # Use False as default for bool
...
active_tbl = db.create_table(
    "active_table_lots_columns", data=data_to_ingest, mode="overwrite"
)
