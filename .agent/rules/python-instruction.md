...
2. Python codes should be strongly typed. To help with that, use the pyrefly toolings. To check if there is any typing errors, please use the `uvx pyrefly check --summarize-errors` command. To automatically infer types using pyrefly, use the `pyrefly infer path/to/directory/` command for directories and `pyrefly infer path/to/file.py` for specific files.
...
5. In python projects, please use pydantic extensively for typesafety. Give easy to understand descriptions regarding the types and other relevant parts.
